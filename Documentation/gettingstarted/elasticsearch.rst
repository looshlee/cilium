...
Sidious has access to perform ``GET`` and ``PUT`` as expected. But see what happens when Vader has both ``GET`` and ``PUT`` access. Vader can completely modify the books! (Note the change in book titles e.g. "Why convert a Jedi!")
