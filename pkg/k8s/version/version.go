...
func leasesFallbackDiscovery(client kubernetes.Interface) error {
	cached.mutex.Lock()
	// Here we check if cilium has authorization to check for leases object or not.
	// Prior to v1.14 leases.coordination.k8s.io/v1 was not supported and hence cilium
	// ClusterRole did not had access to corresponding API resource.
	// Look at the helm manifest to check the ClusterRole associated with cilium Deployments
	// corresponding to the configured Kubernetes version.
	if cached.version.Minor < 14 && cached.version.Major == 1 {
		cached.mutex.Unlock()
		return nil
	}
	...
	case *errors.StatusError:
		if t.ErrStatus.Code == http.StatusNotFound {
			log.WithError(err).Info("Unable to retrieve Leases for kube-system/kube-controller-manager. Disabling LeasesResourceLock")
...
func Update(client kubernetes.Interface, conf k8sconfig.Configuration) error {
	...
	if conf.K8sAPIDiscoveryEnabled() {
		...
		if err != nil {
			...
			if err := leasesFallbackDiscovery(client); err != nil {
				return err
			}
		} else {
